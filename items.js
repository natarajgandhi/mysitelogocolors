...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if (page == 0) {
            ...
            skipCount = itemsPerPage * (page);
        }


        if (category == "All") {
            
            this.db.collection("item").find({}).skip(skipCount).limit(itemsPerPage).sort({"_id":1}).toArray(function (err, results) {
                pageItems = results;
        ...
        this.db.collection("item").find({ "category": category }).skip(skipCount).limit(itemsPerPage).sort({ "category": 1 }).toArray(function (err, results) {
                //console.log(results); // output all records
                pageItems = results;
                callback(pageItems);
            });
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        this.db.collection("item").find({
            ...
            }).skip(skipCount).limit(itemsPerPage).sort({ "_id": 1 }).toArray(function (err, results) {
            items = results;
            //console.log(results); // output all records
            callback(items);
        });
        ...
        //db.item.find({
        //    '$or': [
        //        { 'title': { '$regex': "leaf", '$options': 'i' } },
        //        { 'slogan': { '$regex': "leaf", '$options': 'i' } },
        //        { 'description': { '$regex': "leaf", '$options': 'i' } }]
        //})
    ...
    this.getItem = function(itemId, callback) {
       ...
       //this.db.collection("item").findOne({ "_id": itemId }).toArray(function (err, results) {
       //     //console.log(results); // output all records
       //     //pageItems = results;
       //     console.log(results);
       //     callback(results);
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        this.db.collection("item").update({ _id: itemId }, { $push: { reviews: { "name": name, "comment": comment, "stars": stars, "date": new Date() } } });
        this.db.collection("item").findOne({ _id: itemId }, function (err, document) {
            console.log(document);            
        ...
        //this.db.collection("item").update({ _id: itemId }, { $push: { reviews: { "name": "Nataraj", "comment": "love this", "stars": 5, "date": Date() } } });
